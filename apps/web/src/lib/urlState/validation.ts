...
const VALID_SCENARIO_MODES = new Set<string>(["synthetic", "real"]);
...
export function isValidScenarioMode(value: string | null): value is ScenarioMode {
  if (!value || value.length > MAX_PARAM_LENGTH) return false;
  return VALID_SCENARIO_MODES.has(value);
}
